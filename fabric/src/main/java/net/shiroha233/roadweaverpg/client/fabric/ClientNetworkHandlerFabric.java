...
public class ClientNetworkHandlerFabric {
    ...
    private static void registerProfessionReceivers() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(NetworkHandler.SYNC_PROFESSIONS, (client, handler, buf, responseSender) -> {
            var packet = net.shiroha233.roadweaverpg.network.fabric.message.SyncProfessionsMessage.decode(buf);
        ...
        ClientPlayNetworking.registerGlobalReceiver(NetworkHandler.SYNC_PLAYER_PROFESSION, (client, handler, buf, responseSender) -> {
            var packet = net.shiroha233.roadweaverpg.network.fabric.message.SyncPlayerProfessionMessage.decode(buf);
        ...
        ClientPlayNetworking.registerGlobalReceiver(NetworkHandler.SYNC_RPG_STATS, (client, handler, buf, responseSender) -> {
            var packet = net.shiroha233.roadweaverpg.network.fabric.message.SyncRpgStatsMessage.decode(buf);
            client.execute(() -> ClientStatsCache.updateFromServer(
                    packet.getAttack(), packet.getMaxMana(), packet.getMagicAttack(), 
                    packet.getCritRate(), packet.getCritDamage(),
        ...
        ClientPlayNetworking.registerGlobalReceiver(NetworkHandler.OPEN_PROFESSION_SELECTION, (client, handler, buf, responseSender) -> {
            var packet = net.shiroha233.roadweaverpg.network.fabric.message.OpenProfessionSelectionMessage.decode(buf);
    ...
    public static void sendSelectProfession(ResourceLocation professionId) {
        ...
        new net.shiroha233.roadweaverpg.network.fabric.message.SelectProfessionMessage(professionId).encode(buf);
