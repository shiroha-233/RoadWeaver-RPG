package net.shiroha233.roadweaverpg.network.fabric;
...
public class NetworkHandlerFabric {
    ...
    public static void registerProfessionReceivers() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(NetworkHandler.SELECT_PROFESSION, 
                (server, player, handler, buf, responseSender) -> {
            var packet = net.shiroha233.roadweaverpg.network.fabric.message.SelectProfessionMessage.decode(buf);
        ...
        ServerPlayNetworking.registerGlobalReceiver(NetworkHandler.CHANGE_PROFESSION, 
                (server, player, handler, buf, responseSender) -> {
            var packet = net.shiroha233.roadweaverpg.network.fabric.message.ChangeProfessionMessage.decode(buf);
            server.execute(() -> packet.handle(player));
        });
    ...
    public static void sendRpgStats(ServerPlayer player, 
        ...
        net.shiroha233.roadweaverpg.network.fabric.message.SyncRpgStatsMessage.send(
    ...
    public static void sendPlayerProfession(ServerPlayer player, ResourceLocation professionId) {
        net.shiroha233.roadweaverpg.network.fabric.message.SyncPlayerProfessionMessage.send(player, professionId);
    }
    ...
    public static void sendProfessionDefinitions(ServerPlayer player, 
        ...
        net.shiroha233.roadweaverpg.network.fabric.message.SyncProfessionsMessage.send(player, definitions);
    }
    ...
    public static void sendOpenProfessionSelection(ServerPlayer player, int npcEntityId) {
        net.shiroha233.roadweaverpg.network.fabric.message.OpenProfessionSelectionMessage.send(player, npcEntityId);
    }
}
