package net.shiroha233.roadweaverpg.client.gui.adventure;
...
/**
 * 冒险等级界面
 * 现代化横向时间轴设计，支持滚轮和拖拽滚动
 */
public class AdventureLevelScreen extends Screen {
    ...
    private static final int REWARD_SLOT_SPACING = 4;
    private static final int REWARD_BOX_TOP_MARGIN = 40;
    ...
    private ItemStack hoveredRewardItem = null;
    
    // 数据
    private final Map<Integer, AdventureLevel> levels;
    private final int playerLevel;
    private final int playerExp;
    private int maxLevel;
    
    public AdventureLevelScreen() {
        super(Component.translatable("gui.roadweaver_rpg.adventure.title"));
        this.levels = ClientAdventureCache.getLevelDefinitions();
        this.playerLevel = ClientAdventureCache.getPlayerLevel();
        this.playerExp = ClientAdventureCache.getPlayerExp();
        this.maxLevel = levels.keySet().stream().mapToInt(i -> i).max().orElse(30);
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // 4. 渲染悬停提示（在裁剪区域外，确保始终可见）
        if (hoveredLevel > 0) {
            renderLevelTooltip(graphics, mouseX, mouseY);
        }
        
        // 5. 渲染物品悬停提示
        if (hoveredRewardItem != null) {
            renderItemTooltip(graphics, hoveredRewardItem, mouseX, mouseY);
        }
    ...
    private void renderHeader(GuiGraphics graphics) {
        ...
        // 标题
        Component title = Component.translatable("gui.roadweaver_rpg.adventure.title");
        graphics.drawCenteredString(font, title, width / 2, 15, (alpha << 24) | 0xFFFFFF);
        
        // 当前等级和经验
        String levelText = String.format("Lv.%d", playerLevel);
        String expText = String.format("EXP: %d", playerExp);
        
        int nextLevelExp = ClientAdventureCache.getExpForNextLevel();
        ...
        // 进度条
        if (nextLevelExp > 0) {
            int currentLevelExp = ClientAdventureCache.getExpForCurrentLevel();
    ...
    private void renderTimeline(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int level = 1; level <= maxLevel; level++) {
            ...
            // 只跳过完全在右边界外的节点，左边界外的仍然绘制（会被scissor裁剪）
            if (nodeX > width + 100) break;
            
            // 绘制连接线（只要下一个节点在屏幕内就绘制）
        ...
        for (int level = 1; level <= maxLevel; level++) {
            ...
            // 只跳过完全在右边界外的节点
            if (nodeX > width + 100) break;
            // 左边界外的节点也要绘制（会被scissor裁剪，但奖励框等可能部分可见）
    ...
    private void renderNode(GuiGraphics graphics, int x, int y, int level, int mouseX, int mouseY) {
        ...
        int textColor = isLocked ? 0xFFAAAAAA : 0xFFFFFFFF;
    ...
    private void renderRewardSlots(GuiGraphics graphics, int nodeX, int nodeY, int level,
                                    AdventureLevel levelInfo, boolean completed, boolean nodeHovered,
                                    int mouseX, int mouseY) {
        if (levelInfo == null) return;
        
        // 只获取物品类型的奖励（过滤掉经验等非物品奖励）
        ...
        int startY = nodeY + NODE_RADIUS + REWARD_BOX_TOP_MARGIN;
    ...
    private void renderItemRewardSlot(GuiGraphics graphics, int x, int y, ItemReward itemReward, 
        ...
        GuiRenderer.drawRoundedRect(graphics, x, y, REWARD_SLOT_SIZE, REWARD_SLOT_SIZE, 6, bgColor);
        ...
        // 渲染物品图标
        ItemStack stack = itemReward.createDisplayStack();
        if (!stack.isEmpty()) {
            ...
            if (stack.getCount() > 1) {
                ...
                graphics.drawString(font, countStr, 
                        x + REWARD_SLOT_SIZE - font.width(countStr) - 2,
                        y + REWARD_SLOT_SIZE - 10, 0xFFFFFFFF, true);
        ...
        if (completed) {
            ...
            graphics.drawCenteredString(font, "✓", x + REWARD_SLOT_SIZE / 2, y + REWARD_SLOT_SIZE / 2 - 4, 0xFF32CD32);
            graphics.pose().popPose();
        }
    }
    ...
    private void renderLevelTooltip(GuiGraphics graphics, int mouseX, int mouseY) {
        AdventureLevel levelInfo = levels.get(hoveredLevel);
        if (levelInfo == null) return;
        ...
        tooltip.add(Component.literal("§6冒险等级 " + hoveredLevel + statusStr));
        
        // 所需经验
        tooltip.add(Component.literal("§7所需经验: §f" + levelInfo.getRequiredExperience()));
