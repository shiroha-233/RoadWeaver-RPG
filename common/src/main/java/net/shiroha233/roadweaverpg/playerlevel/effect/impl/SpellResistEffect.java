 ...
 */
public class SpellResistEffect implements LevelEffect {
    
    private final double amount;
    
    public SpellResistEffect(double amount) {
        this.amount = amount;
    }
    
    @Override
    public String getTypeId() {
        return LevelEffectType.SPELL_RESIST.getId();
    }
    
    @Override
    public Component getDescription() {
        return Component.translatable("effect.roadweaver_rpg.spell_resist", 
                String.format("+%.1f%%", amount));
    }
    
    @Override
    public void apply(ServerPlayer player, int level) {
        StatEffectService.applySpellResist(player, amount);
    }
    ...
    public static SpellResistEffect fromJson(JsonObject json) {
        double amount = json.has("amount") ? json.get("amount").getAsDouble() : 5.0;
        return new SpellResistEffect(amount);
    }
    
    public static SpellResistEffect fromNetwork(FriendlyByteBuf buf) {
        return new SpellResistEffect(buf.readDouble());
    }
